...
REPO_ID = "gowthamd94/tourism-dataset"

def prepare_data():
    print("--- [2] Data Preparation Started ---")
    try:
        # Rubric: Load dataset directly from Hugging Face data space
        print("Loading data from Hugging Face...")
        path = hf_hub_download(repo_id=REPO_ID, filename="raw/tourism.csv", repo_type="dataset", token=HF_TOKEN)
        df = pd.read_csv(path)

        # Rubric: Perform data cleaning and remove unnecessary columns
        print("Cleaning data...")
        ...
        # Rubric: Split cleaned dataset into train/test and save locally
        print("Splitting data...")
        train, test = train_test_split(df, test_size=0.2, random_state=42)
        train.to_csv("train.csv", index=False)
        test.to_csv("test.csv", index=False)

        # Rubric: Upload resulting train/test datasets back to HF data space
