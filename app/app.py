...
# --- CONFIG ---
MODEL_REPO = "gowthamd94/tourism-model"

# Rubric: Load the saved model from Hugging Face model hub
@st.cache_resource
def load_model():
...
st.title("🌴 Tourism Package Prediction AI")

model = load_model()
if model:
    st.success("AI System Connected (Model Loaded from HF Hub)")
else:
    st.error(" Failed to load model. Check HF Token and Repo.")

# Rubric: Get inputs and save them into a dataframe
